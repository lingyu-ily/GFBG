...
《情書》每回合由伺服器自動抽牌，玩家選手牌、目標與猜測後確認出牌；大臣需選保留牌及牌庫底順序。《暗影獵人》依提示完成移動、可選地點行動與可選攻擊；身分、隱士牌及未使用角色均由伺服器依玩家視角過濾。飛行船版使用獨立角色與牌庫：擲出 10 登船，外圍攻擊範圍沿逆時針延伸，推理牌內容只傳給抽牌者與接收者。對局結束後可回大廳再開新房。
