...
繁體中文私人桌遊平台，收錄新版《情書》、《暗影獵人》與《暗影奇襲：女王陛下的飛行船》。手機／電腦共用牌桌，訪客可直接入房，註冊會員可保存完整對局戰績並設定會員頭像。
...
開啟終端顯示的網址（預設 `http://localhost:3000`）。使用兩個不同瀏覽器或一般／無痕視窗可測試不同玩家；相同瀏覽器分頁會共用身份。預覽未設定 SMTP 時仍可註冊及登入，帳號會顯示 Email 尚未驗證並可稍後重寄。
...
`.env` 限制為管理者可讀，勿加入 Git。`HOST_PORT` 預設 3000，可調整為未占用埠。正式環境必須是 HTTPS，Email 驗證與密碼重設連結使用固定的 PUBLIC_URL 產生。未提供 SMTP 主機或寄件者時訪客、註冊與登入仍正常，但無法驗證 Email 或重設遺失的密碼。六項頭像設定全空時停用頭像上傳；只填部分會拒絕啟動並列出缺少項目。
...
4. 以登入帳號、顯示名稱、Email 與密碼註冊；在另一個瀏覽器開啟驗證信，確認只驗證信箱而不登入，再以登入帳號與密碼登入並接回原座位。
...
房間預設為私人；開房時可選擇公開，房主之後也能隨時切換。公開房會出現在首頁，等待中的房間可直接入座，已開局或滿房時仍可旁觀。私人房不會出現在首頁，但知道 8 碼房號的人可以透過旁觀連結觀看。旁觀者必須設定暱稱，只會收到公開資訊，不占座位、不影響房主轉移或戰績；同一身份的多個分頁只計一次，每房最多 50 位在線旁觀者。
...
訪客身份存於安全 Cookie。重新整理、關閉後重開同一瀏覽器可恢復；清除 Cookie 或換裝置不能憑暱稱取回訪客座位。會員使用不分大小寫的登入帳號與密碼登入；Email 只用於驗證與密碼重設。登入玩家可在另一瀏覽器登入後，透過大廳「你的牌桌還在」重新加入原座位。登出會撤銷目前 session，之前的座位仍屬原帳號。
