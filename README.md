...
繁體中文私人桌遊平台，收錄新版《情書》與《暗影獵人》。手機／電腦共用牌桌，訪客可直接入房，Email 登入保存完整對局戰績。
...
所有人準備後才能開始，房間開局後鎖定座位。《情書》由房主指定第一輪先手，後續由本輪勝者先手，平手在勝者間隨機選擇；每輪結束由房主開始下一輪。《暗影獵人》在 4–8 人準備完成後隨機決定先手。

《情書》每回合由伺服器自動抽牌，玩家選手牌、目標與猜測後確認出牌；大臣需選保留牌及牌庫底順序。《暗影獵人》依提示完成移動、可選地點行動與可選攻擊；身分、隱士牌及未使用角色均由伺服器依玩家視角過濾。對局結束後可回大廳再開新房。
...
規則測試涵蓋《情書》10 種角色與 21 張牌，以及《暗影獵人》20 名角色、三副各 16 張牌、隱藏資訊和 4／6／8 人完整模擬。整合測試使用獨立的真實 PostgreSQL 與本機 SMTP 接收器，執行 HTTP／WebSocket 客戶端、兩款遊戲的完整對局、登入、交易競爭及故障恢復；不寄送外部郵件，不使用正式環境 DATABASE_URL。
